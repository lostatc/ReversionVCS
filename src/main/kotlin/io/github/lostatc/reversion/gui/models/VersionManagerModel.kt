...
class VersionManagerModel : CoroutineScope by MainScope() {
    ...
    fun setFile(path: Path) {
        // TODO: Properly handle an [OpenAttempt.Failure].
        val workDirectory = WorkDirectory.openFromDescendant(path).onFail {
            error("The working directory could not be opened.")
        }
        selectedFile = storageActor.wrap(FileState(path, workDirectory))
        loadVersions()
    }
