...
class Config {
    ...
    /**
     * The set of [keys][ConfigProperty.key] contained in this config.
     */
    val keys: Set<String>
        get() = properties.map { it.key }.toSet()

    /**
     * Sets the [value] of the given [property] in this config.
     *
     * @throws [IllegalArgumentException] A different property with the same key already exists in this config.
     */
